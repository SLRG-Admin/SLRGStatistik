...
const AttendanceModal = ({ training, onClose, allTrainers }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            await db.transaction('rw', db.attendances, db.trainings, async () => {
                ...
                if (attendees.size > 0) {
                    const newAttendances = Array.from(attendees).map((member_id) => ({
...
const TrainingFormModal = ({ onClose, allTrainers }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        await db.trainings.add(newTraining);
        onClose();
    };
