...
// This is a common type. Can be either Member or Trainer.
// Both have the same properties so we can treat them similarly in generic components.
type Person = Member | Trainer;
...
const PersonForm: React.FC<{
    ...
    onCancel: () => void;
    entityName: string;
}> = ({ person, onSave, onCancel, entityName }) => {
...
const PersonManager: React.FC<{
    ...
    entityName: string;
}> = ({ title, people, onAdd, onUpdate, onDelete, entityName }) => {
...
const Management: React.FC = () => {
    ...
    const handleAddMember = async (member: Omit<Member, 'id'>) => {
        await db.members.add(member as Member);
    };
    const handleUpdateMember = async (id: number, updates: Partial<Member>) => {
    ...
    const handleAddTrainer = async (trainer: Omit<Trainer, 'id'>) => {
        await db.trainers.add(trainer as Trainer);
    };
    const handleUpdateTrainer = async (id: number, updates: Partial<Trainer>) => {
